...
class Tournament(models.Model):
    default_desc = """
    Вначале каждый участник играет 6-10 игр (в зависимости от количества участников) со случайными соперниками. 
    Одна игра состоит из 5 коротких партий (вне зависимости от количества побед каждого из игроков). 
    За каждую победную партию игроку присуждается 1 очко. После всех игр данного этапа составляется рейтинг участников 
    и первые 8 игроков в рейтинге выходят в плей-офф. В случае равенства очков по количеству выигранных партий 
    учитывается разница забитых и пропущенных мячей. В случае равенства и этого показателя учитывается результат личной 
    встречи, если таковая была. Если игроки на данном этапе не встречались, назначается дополнительная игра до 3 побед 
    в коротких партиях. </br></br>

    Каждая игра этапа плей-офф играется до 4 побед в коротких партиях. </br></br>

    NEW: Участники турнира должны сами отправлять результаты своих игр через "личный кабинет" на данном сайте. 
    Достаточно, чтобы это сделал один из участников каждый игры. Рейтинг будет обновляться автоматически.</br></br>
    """
    debug = models.BooleanField('if debug mode', default=False)
    participants = models.ManyToManyField(User, through='Participant')
    description = models.TextField('description', default=default_desc)
